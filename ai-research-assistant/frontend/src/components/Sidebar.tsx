...
export function Sidebar({ user, onLoginClick, onLogout, onNewChat, onSelectConversation }: SidebarProps) {
  ...
  return (
    <motion.aside
      ...
      className="h-screen border-r border-white/5 flex flex-col bg-dark-300/50"
    >
      {/* Toggle Button */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className="absolute -right-3 top-6 z-10 w-6 h-6 rounded-full bg-dark-100 border border-white/10 flex items-center justify-center text-gray-400 hover:text-white transition-colors"
      >
        {isCollapsed ? (
